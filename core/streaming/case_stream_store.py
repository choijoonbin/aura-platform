...
# Audit Analysis: case_id -> finalResult (reasonText, proposals, confidenceBreakdown)
_audit_analysis_results: dict[str, dict] = {}

_case_stream_store: CaseStreamStore | None = None


def set_audit_analysis_result(case_id: str, result: dict) -> None:
    """감사 분석 결과 저장 (GET /analysis에서 반환)"""
    _audit_analysis_results[case_id] = result


def get_audit_analysis_result(case_id: str) -> dict | None:
    """감사 분석 결과 조회"""
    return _audit_analysis_results.get(case_id)
